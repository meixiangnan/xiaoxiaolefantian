...
namespace Watermelon
{
    public class EditorSceneController
    {
        ...
        public void SetSortingOrder(GameObject prefab, int order)
        {
            ...
            for (int i = 0; i < spriteRenderers.Length && i < 2; i++)
            {
                var render =  spriteRenderers[i];
                if (render.gameObject.name.Equals("Icon"))
                {
                    render.sortingOrder = order * 3 - 1;
                }
                else if (render.gameObject.name.Equals("Back"))
                {
                    render.sortingOrder = order * 3 - 2;
                }
            }
